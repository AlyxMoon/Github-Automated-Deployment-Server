...
const createOrUpdateRepository = (repositoryInfo) => {
  fs.readdir(WEB_DIRECTORY)
    .then(files => {
      console.log('TEST', WEB_DIRECTORY, files, repositoryInfo.name, files.includes[repositoryInfo.name])
      return files.includes[repositoryInfo.name] ? updateRepository(repositoryInfo) : createRepository(repositoryInfo)
    })
